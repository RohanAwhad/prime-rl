#!/usr/bin/env python3
"""
Remote deployment script for cascade LLM training.

This script:
1. Creates a git archive of the current codebase
2. Transfers it to the remote GPU node
3. Extracts the archive
4. Launches the cascade workload in a tmux session

Usage:
    python scripts/deploy_cascade.py <node-id>
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Deploy cascade LLM training to a remote GPU node")
    parser.add_argument("node_id", help="Remote node ID (e.g., rh-h100-09)")
    args = parser.parse_args()

    remote_host = args.node_id

    print("=" * 60)
    print(f"Cascade LLM Training - Remote Deployment to {remote_host}")
    print("=" * 60)
    ...
    # Step 6: Transfer cascade run script to remote
    local_run_script = Path(__file__).parent / "run_cascade.sh"
    ...
    run_command(
        ...
        "Transferring run_cascade.sh to remote"
    )

    # Step 7: Make run script executable
    run_command(
        ["ssh", remote_host, f"chmod +x {REMOTE_PROJECT_DIR}/run_cascade.sh"],
        "Making run_cascade.sh executable"
    )
    ...
    # Step 9: Launch tmux session with cascade run script
    ...
    launch_cmd = (
        ...
        f"tmux new-session -d -s {TMUX_SESSION_NAME} '{env_prefix}./run_cascade.sh'"
    )
    run_command(
